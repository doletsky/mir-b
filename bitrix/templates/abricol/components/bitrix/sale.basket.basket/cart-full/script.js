...
function updateBasketTable(basketItemId, res)
{
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED'))
			BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/руб./g, '').replace(/\s/g, '&nbsp;')+' <span class="rub">i</span>';
		if (BX('allSum_FORMATED1'))
			BX('allSum_FORMATED1').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/руб./g, '').replace(/\s/g, '&nbsp;')+' <span class="rub">i</span>';
...
{	
	...
	if (bIsCorrectQuantityForRatio)
	{
		...
		// lex sv
		var row=$("#"+controlId).closest(".cart_tab_row");
		var price=parseInt(row.find(".col_td_3").text().replace(/\s/g,''));
		row.find(".col_td_4").html(number_format(newVal*price,0,'',' ')+' <span class="rub">i</span>');
...
function recalcBasketAjax(params)
{
	...
	// lex sv
	var rows=$("#basket_items .cart_tab_row");
	var rowsLength=rows.length;
	//if (!!items && items.rows && items.rows.length > 0)
	rows.each(function(){
		var $ID=$(this).attr("id");
		postData['QUANTITY_' + $ID] = BX('QUANTITY_' + $ID).value;
	});

	if(rowsLength)
	{
		for (i = 1; rowsLength > i; i++)
