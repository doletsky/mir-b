...
<?if(!empty($arResult['TOP_TEXT']) && !preg_match("#PAGEN_[^=]*\=([\d]*)#", $_SERVER['REQUEST_URI'])):?>
	<?$this->SetViewTarget("category-text-top");?>
	<div class="seo-text">
		<?=$arResult['TOP_TEXT'];?>
	</div>
	<?$this->EndViewTarget();?>
...
<ul class="catalogue_list">
	...
	<?#pre($arItem);?>
	<li>
    <div href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="cat_box">
    	<?if($arItem["PROPERTIES"]["IS_NEW"]["VALUE_XML_ID"]=="Y"){?><span class="spec_badge spec">Новинка</span><?}?>
    	<?if($arItem["PROPERTIES"]["IS_SPEC"]["VALUE_XML_ID"]=="Y"){?><span class="spec_badge spec">Спецпредложение</span><?}?>
		<span class="cat_name">
			<span class="strong"><a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?=$arItem["NAME"]?></a></span>
		</span>
		<a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><img src="<?=MakeImage($arItem["PREVIEW_PICTURE"]["SRC"],array("w"=>290,"h"=>225))?>" alt="" /></a>
			
      <span class="price2">от <?=number_format($arItem["MIN_PRICE"]["VALUE"],0,'.',' ')?> <span class="rub">i</span></span>
			<?
			if(in_array($arItem["PROPERTIES"]["LUMINAIRE_TYPE"]["VALUE_ENUM_ID"],array(53,54))){
				#echo '2222';
				$LAMPSHADES=array();
				foreach($arItem["OFFERS"] as $offer){
					#pre($offer["PROPERTIES"]["LAMPSHADES"]);
					$LAMPSHADES[$offer["PROPERTIES"]["LAMPSHADES"]["VALUE_XML_ID"]]=1;
				}
				#pre($LAMPSHADES);
				if(count($LAMPSHADES)){
					?>
		      <span class="lamp_nums">
		      	<?if(isset($LAMPSHADES[3])){?><span><b></b>x3</span><?}?>
		        <?if(isset($LAMPSHADES[4])){?><span><b></b>x4</span><?}?>
		        <?if(isset($LAMPSHADES[5])){?><span><b></b>x5</span><?}?>
		        <?if(isset($LAMPSHADES[6])){?><span><b></b>x6</span><?}?>
		      </span>
		      <?
		    }
    	}
...
<?if(!empty($arResult['BOT_TEXT']) && !preg_match("#PAGEN_[^=]*\=([\d]*)#", $_SERVER['REQUEST_URI'])):?>
	<div class="seo-text">
		<?=$arResult['BOT_TEXT'];?>
	</div>
<?endif;?>
