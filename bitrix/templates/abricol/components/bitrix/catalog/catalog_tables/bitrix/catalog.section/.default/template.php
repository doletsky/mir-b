...
<ul class="catalogue_list">
	<?
	$sizeID = (isset($_REQUEST["filter"][9]) && $_REQUEST["filter"][9])?(int)$_REQUEST["filter"][9]:null;
	?>
	<?foreach($arResult["ITEMS"] as $arItem){?>
		<!--<?#if($arItem["ID"]==27) pre($arItem);?>-->
		<?
		$minPrice = 99999999;
		foreach( $arItem["OFFERS"] as $arOffer ){
			$offerSizeID = $arOffer["PROPERTIES"]["SIZE"]["VALUE_ENUM_ID"];
			$offerPrice = $arOffer["PRICES"]["BASE"]["VALUE"];
			if( $offerSizeID && $sizeID && $offerSizeID!=$sizeID ) continue;
			if( $offerPrice<$minPrice ) $minPrice = $offerPrice;
		}
	    ...
	    <div href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="cat_box">
	      ...
	      <span class="price2">от <?=number_format($minPrice,0,'.',' ')?> <span class="rub">i</span></span>
	    </div>
	  </li>
	<?}?>
</ul>
