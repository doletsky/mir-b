...
$userfields = CIBlockSection::GetList(Array(), Array('IBLOCK_ID'=>$arResult['IBLOCK_ID'], 'ID' => $arResult["ID"]), false, array('UF_*'))->fetch();

if(!empty($userfields['UF_SEO_H1'])){
	$cp = $this->__component;

	if (is_object($cp)){
		 $cp->arResult['SEO_H1'] = $userfields['UF_SEO_H1'];
		 $arResult['SEO_H1'] = $cp->arResult['SEO_H1'];

		 $cp->SetResultCacheKeys(array('SEO_H1'));
	}
}

if(!empty($userfields['UF_SEO_TEXT_TOP'])){
	$arResult['TOP_TEXT'] = $userfields['UF_SEO_TEXT_TOP'];
}
if(!empty($userfields['UF_SEO_TEXT_BOT'])){
	$arResult['BOT_TEXT'] = $userfields['UF_SEO_TEXT_BOT'];
}
?>