...
<div class="form_box">
  ...
  <!--label class="label_radio" for="delivery_1"><input class="pay-del__deliver_1 pay-del__deliver" name="delivery" id="delivery_1" value="1" type="radio" />по Екатеринбургу</label>
  <label class="label_radio" for="delivery_2"><input class="pay-del__deliver" name="delivery" id="delivery_2" value="2" type="radio" />в регионы РФ</label>
  <label class="label_check" for="check_1"><input name="check" id="check_1" value="1" type="checkbox" />Крупногабаритный груз <em>стол, мебель, настольные игры <br>от 50 кг</em></label-->
	...
	if(!empty($arResult["DELIVERY"]))
	{
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			if ($delivery_id !== 0 && intval($delivery_id) <= 0)
			{
				...
				{
					?>
					<label for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>" class="label_check">
							<input
								type="radio"
			...
			else // stores and courier
			{
				...
				?>
					<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?> class="label_radio">
...
foreach($arResult["ORDER_PROP"]["USER_PROPS_N"] as $arProp){
	if($arProp['CODE'] == 'BIG_WEIGHT'){
	 ?>
	 
	  <label class="label_check" for="<?=$arProp["FIELD_NAME"]?>"><input value="Y" <?if ($arProp["CHECKED"]=="Y") echo " checked";?> name="<?=$arProp["FIELD_NAME"]?>" id="<?=$arProp["FIELD_NAME"]?>"  type="checkbox" /><?=$arProp['NAME']?> <em><?=$arProp['~DESCRIPTION']?></em></label>
	 <?
	}
}
